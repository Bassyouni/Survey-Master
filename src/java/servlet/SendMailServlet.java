...
public class SendMailServlet extends HttpServlet {
    ...
    public void sendMail(HttpServletRequest request, HttpServletResponse response){
        String to = request.getParameter("targetUserMail");
        String subject = request.getParameter("subject");
        String message = request.getParameter("message");
        if(to.equals("all")){// to all users
            ArrayList<User> users = User.selectAllUsers();
            InternetAddress[] adress = new InternetAddress[users.size()];
            for(int i = 0; i < users.size(); i++){
                adress[i] = new InternetAddress();
                adress[i].setAddress(users.get(i).getEmail());
            }
            SendMail.sendAll(adress, subject, message);
        }
        else{//to target user
